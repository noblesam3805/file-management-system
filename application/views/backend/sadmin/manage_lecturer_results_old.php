...
<div class="row">
	...
	$query =mysql_query("select course_assigned_to_dept_id from course_assigned_to_lecturers where lecturer_id = '$lecturerid' and session='$session' and semester='$semester'") or die("1");
	while(list($cid) = mysql_fetch_array($query))
								{
	 $query1 =mysql_query("select id,course_id,course_unit,course_type_id,departmt from course_assigned_to_department where id='$cid'") or die(mysql_error());
								while(list($id1,$courseid,$course_unit,$course_type_id,$departmt) = mysql_fetch_array($query1))
								{?>
         <tbody> 
           <tr bgcolor="#E2E2E2">
             <td>&nbsp;</td>
    ...
    <span id="myText<?php echo $id1;?>"></span>
          ...
          <td align="center">
            ...
            <?php
			$query33 =mysql_query("select has_result from course_assigned_to_department where id = '$id1'") or die("1");
	while(list($has_result) = mysql_fetch_array($query33))
								{
			 if($has_result==1)
			{
				?>
                  <a href="<?php echo base_url().'index.php?sadmin/download_uploaded_results_marksheets/'.$id1;?>"  target="_blank">View/Print Mark Sheet</a><?php
			}
			else
			{?>
