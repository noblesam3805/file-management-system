...
$data['level'] = $yr;
...

    {
       ...
       // $page_data['hostel'] = $this->db->order_by('id', 'DESC')->get('counter',3)->result_array();
        $page_data['etz'] = $this->db->order_by('payee_id', 'DESC')->get('nekede_etranzact_payment',3)->result_array();
    ...
    function generate_payment_reports()
	{
        ...
        $page_data['page_title'] ="Generate FPN Payment Reports";
	...
	function processSchoolPayments()
	{
		...
		$page_data['id']  = $payment_type;
		$page_data['session']  = $session;
		if($payment_type==7 || $payment_type==10)
		{
		$page_data['page_name']  = 'payment_reports_morning';
	    }
		else
		{
			$page_data['page_name']  = 'payment_reports_others';
		}

        $page_data['page_title'] = $this->db->get_where('application_type', array("application_typeid" => $payment_type))->application_type;

        $this->load->view('backend/sadmin_print', $page_data);
	}
...
$page_data = array(
  ...
  $this->load->view('backend/index', $page_data);
   ...
   $this->load->view('backend/index', $page_data);
    ...
    function etranzact($param1 = '', $param2 = '', $param3 = '')
    {
        if ($this->session->userdata('sadmin_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($param1 ==''){
            ...
            $page_data['invoices'] = $this->db->get('etranzact_pay')->result_array();
             $page_data['total'] = $this->db->get('etranzact_pay')->num_rows();
        }
        ...
        //if($this->etranzact->get_status() != "-1"){
        $data['customer_id'] =  $this->input->post('serial');
        $data['date_reg'] =  date("Y-m-d"); //date registered
        $data['fullname'] =  $this->input->post('name').",".$this->input->post('oname');
        $data['phone'] =    $this->input->post('phone');
        $data['prog_type'] =      $this->input->post('prog_type');
        $data['dept'] =      $this->input->post('dept');
        $data['level'] =      $this->input->post('level');
        $data['session'] =      $this->input->post('session');
        $data['cust_add'] =     $this->input->post('address');
        $data['used_by'] =      $this->input->post('serial');
        $data['status'] =       "paid";

        $invoice_id = $this->input->post('CONFIRMATION_NO');
        $this->db->where('confirm_code', $invoice_id);
        $this->db->update('etranzact_payment', $data);
		redirect(base_url() . 'index.php?sadmin/not_paid', 'refresh');
        ...
        $page_data['page_title'] = get_phrase('E-tranzact_payment_list');
