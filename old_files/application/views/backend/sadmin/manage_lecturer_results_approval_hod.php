...
<div class="row">
	...
	<div class="col-md-12">
	  ...
	  <?php 
	  $id=1;
	  $dean_id=$this->session->userdata('sadmin_id');
	  $dean_dept = $this->db->query("select* from sadmin where sadmin_id='$dean_id'")->row()->dept_id;
	 
	  $dean_school = $this->db->query("select* from department where deptID='$dean_dept'")->row()->schoolid;
	  
	$query =sqlsrv_query($conn,"select lecturer_id, course_assigned_to_dept_id from course_assigned_to_lecturers where session='$session' and semester='$semester'") or die("1");
	while(list($lect_id,$cid) = sqlsrv_fetch_array($query))
		
					{
		$lecturer_dept = $this->db->query("select* from sadmin where sadmin_id='$lect_id'")->row()->dept_id;
		
		 $lecturer_school = $this->db->query("select* from department where deptID='$lecturer_dept'")->row()->schoolid;
		 if($dean_school != $lecturer_school)
		 {
		 }
		 else{
		 
									$lecturer_name = $this->db->query("select* from sadmin where sadmin_id='$lect_id'")->row()->name;
	 $query1 =sqlsrv_query($conn,"select id,course_id,course_unit,course_type_id,departmt from course_assigned_to_department where id='$cid'") or die('2');
         ...
         <tbody> 
          ...
          <td><div align="left"><span class="style5">
            <?php $q1=sqlsrv_query($conn,"select *  from eduportal_courses where course_id='$courseid'")or  die("3");
		  
		  while(list($courseid1,$coursecode1,$course_title1)=sqlsrv_fetch_array($q1))
		  {echo  $coursecode1;}?>
            (<?php $q2=sqlsrv_query($conn,"select *  from eduportal_courses where course_id='$courseid'")or  die("5");while(list($courseid2,$coursecode2,$course_title2)=sqlsrv_fetch_array($q2))
    ...
    <span id="myText<?php echo $id1;?>"></span>
          ...
          <td align="center">
            ...
            <?php
			$query33 =sqlsrv_query($conn,"select has_result from course_assigned_to_department where id = '$id1'") or die("4");
