...
{
var check = confirm("Do you want to delete this student record?");
if(check==true)
{
...
$deptopt = $this->db->query("select *  from dept_options where dept_option_id='".$row['dept_option_id']."'")->row()->dept_option_name;
							...
							echo $deptopt;?></td>
                            <td><?php 
							$stype = $this->db->query("SELECT * FROM student_type WHERE student_type_id=".$row['student_type_id']."")->row()->student_type_name;
							echo $stype;?></td>
							<td><a href="#index.php?sadmin/editadmislist/<?php echo $row['id'];?>" class="btn btn-danger btn-sm " >
                       Edit
                     </a>| <a href="index.php?sadmin/delete_studentlist/<?php echo $row['id'];?>" class="btn btn-success btn-sm "  onclick="checkdel()">
                        Delete  </a></td>
                            <?php $i++;?>
