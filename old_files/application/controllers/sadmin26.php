...
<td><div align='center'><span class='style5'>DUPLICATE $check_dup->application_no</span></div></td>
								...
								else
								{
									...
									sqlsrv_query($conn,"insert into eduportal_admission_list (application_no,surname,firstname,middlename,dept_id,school_id,dept_option_id,student_type,programme_type_id,session,admissionlist_batch_id,activated,user_id,adm_type) values ('$jamb','$surname','$firstname','$middlename','$depts','$school','$deptsoptions','$programme','$programme_type_id','$sess','$batch','1','$sadmin_id','$ltype')") or die ("5");
							//	mysql_query("insert into `YABATECHedu1_admission`.`eduportal_admission_list` (application_no,surname,firstname,middlename,dept_id,school_id,dept_option_id,student_type,programme_type_id,session,admissionlist_batch_id,activated,user_id,adm_type) values ('$jamb','$surname','$firstname','$middlename','$depts','$school','$deptsoptions','$programme','$programme_type_id','$sess','$batch','1','$sadmin_id','$ltype')") or die (mysql_error()."4");
								$uploadedfile= $uploadedfile." <tr>
          <td><div align='center'><span class='style5'> $i </span></div></td>
          <td><div align='left'><span class='style5'>$jamb</span></div></td>
          <td><div align='center'><span class='style5'>$surname</span></div></td>
...
echo		$_SESSION["error"];						
   ...
   $this->db->where('application_no', $id);
			...
			//$this->db->where('application_no', $id);
         //   $this->db->update('YABATECHedu1_admission.eduportal_admission_list', $data);
...
$this->load->view('backend/index',$page_data);
 ...
 public function screen_applicants()
   {
   	session_start();
      if ($this->session->userdata('sadmin_login') != 1)
   ...
   //$page_data['accounts'] = $this->db->query("select* from yabatech_application_portal.hnd_applicants_form where progress_step='3'")->result_array();
   
   $page_data['page_name']  = 'view_report_by_department';
   $page_data['page_title'] = ' Screen Applicants';
 ...
 public function verify_olevel_result()
    ...
    public function process_verify_olevel_result()
   {
	...
	//$PrivateKey =openssl_pkey_get_private("jHCqGUeK");
	$privatekey = "jHCqGUeK";
...
$ivlen = openssl_cipher_iv_length($method);
    ...
    $EncodedeCandidateNo = openssl_encrypt($eCandidateNo, $method, $PrivateKey,$options=0, $iv);
	
	$EncodedePassword = openssl_encrypt($ePassword, $method, $PrivateKey,$options=0, $iv);
	
	$EncodedeeSecToken = openssl_encrypt($eSecToken, $method, $PrivateKey,$options=0, $iv);
	
	$EncodedeExamYear = openssl_encrypt($eExamYear, $method, $PrivateKey,$options=0, $iv);
...
//header("Content-Type: text/xml");
 ...
 <soap:Body> 
 <CheckResultJSON xmlns="http://WAEC.org/">       
 <Username>'.$Username.'</Username>       
 <ePassword>'.$EncodedePassword.'</ePassword>       
 <eCandidateNo>'.$EncodedeCandidateNo.'</eCandidateNo>       
 <eExamYear>'.$EncodedeExamYear.'</eExamYear>       
 <ExamType>'.$ExamType.'</ExamType>       
 <eSecToken>'.$EncodedeeSecToken.'</eSecToken>     
...
$url = 'https://waeconline.org.ng/verification/subscribe';
 
/* //Initiate cURL
$curl = curl_init($url);
 
//Set the Content-Type to text/xml.
curl_setopt ($curl, CURLOPT_HTTPHEADER, array("Content-Type: text/xml; charset=utf-8","Host: waeconline.org.ng","Content-Length: 10000","SOAPAction: 'http://WAEC.org/CheckResultJSON'"));
 ...
 */
 ...
 try{
$soapclient = new SoapClient('https://waeconline.org.ng/verification/subscribe');
$param=array('Username'=>$Username,'ePassword'=>$EncodedePassword,'eCandidateNo'=>$EncodedeCandidateNo,'eExamYear'=>$EncodedeExamYear,'ExamType'=>$ExamType,'eSecToken'=>$EncodedeeSecToken);
