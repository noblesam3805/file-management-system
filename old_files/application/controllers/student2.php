...
if($account_status=="1")
	...
	public function adm_letter(){
		...
		$page_data['page_title'] = '2018/2019 EBSU ('.substr($_SESSION["progCode"],-1).') ADMISSION LETTER';
...
function generate_other_fees_invoice(){
	...
	function processOtherFeesInvoice()
	{
		...
		$beneficiaryName2="EBSU Treasury Single Account (CBN)";
...
$this->db->insert('invoice_gen',$invoice_data);	
  $_SESSION['invoice_no']=   $rrr;
		$page_data['page_name']  = 'invoice_page';
        $page_data['page_title'] = 'FPN '.$payment_description.' Invoice';
	...
	public function other_fees_invoice()
{
	...
	if ($this->session->userdata('student_login') != 1)
            redirect(base_url(), 'refresh');
$page_data['page_title'] = 'FPN '.$_SESSION['fee_description'].' Invoice';
...
$this->db->insert('invoice_gen',$invoice_data);	
  $_SESSION['invoice_no']=   $rrr;
		$page_data['page_name']  = 'invoice_page';
        $page_data['page_title'] = 'FPN MSCP Invoice';
...
$invoice_code="FPN/".$applicationcategory_code->application_code."/16".$lpad_invoice_id;
$paymentamount=5000;
	...
	public function mscp_invoice()
{
	if ($this->session->userdata('student_login') != 1)
            redirect(base_url(), 'refresh');
$page_data['page_title'] = 'FPN Microsoft Collaboration Platform Fee Invoice';
...
$page_data['page_name']  = 'remita_mscp_fee_receipt';
    $page_data['page_title'] = 'FPN MSCP Fee Payment Receipt';
$this->load->view('backend/student_print',$page_data);
 }	
...
$page_data['page_name']  = 'etransact_mscp_fee_receipt';
    $page_data['page_title'] = 'FPN Microsoft Collaboration Platform Fee Payment Receipt';
$this->load->view('backend/student_print',$page_data);
...
function generate_tedc_fee_invoice(){
        ...
        $page_data['page_title'] = get_phrase('Get HOSTEL Fee Invoice');
        $page_data['my_data']  = $this->db->get_where('student', array(
            'student_id' => $this->session->userdata('student_id')
        ))->row();
...
function processTedcInvoice($rrr)
	{
...
if(!$rrr)
{
		...
		$_SESSION["progtype"] = $progtype;
		$_SESSION["prog"] = $prog;
		
		$_SESSION["programme_type_id"]=$this->input->post('programme_type_id');
...
define("MERCHANTID", "2898067830");
define("SERVICETYPEID", "538814560");
define("APIKEY", "405978");
...
define("PATH", 'http://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']));
 ...
 $_SESSION["orderID"]=$orderID ;
		...
		$payerEmail = $studentDetails->email == "" ? 'info@fedpolybonny.edu.ng' : $studentDetails->email;  

		$payerPhone = $studentDetails->phone; 

		$responseurl = "http://104.217.252.100/portal/RemitaGatewayHOSTEL.php";
...
$mobile_no = $studentDetails->phone;
		...
		$application_form_category = $_SESSION["programme_type_id"];
$application_exist = $this->db->get_where('invoice_gen', array("portal_id" => $portalID,"application_type_id"=>$application_form_category,"session_id" => $session,"level" => $year))->row();
if($application_exist)
{
	$invoice_data = array(
...
$invoice_data = array(
			   ...
			   'email' => $email,
			   'dept_id' => $studentDetails->dept,
			   'fee_description'=>'YEAR I - HOSTEL FEE',
			   'level'=>$year,
			   'semester'=>$semester
...
$this->db->insert('invoice_gen',$invoice_data);	
  $_SESSION['invoice_no']=   $rrr;
		$page_data['page_name']  = 'invoice_page';
        $page_data['page_title'] = 'EBSU HOSTEL Fee Payment Invoice';
	...
	public function tedc_invoice()
{
	if ($this->session->userdata('student_login') != 1)
            redirect(base_url(), 'refresh');
$page_data['page_title'] = 'EBSU HOSTEL Fee Invoice';
...
function pay_tedc_fees(){
        ...
        $page_data['page_title'] = get_phrase('Pay HOSTEL Fees');
        $page_data['my_data']  = $this->db->get_where('student', array(
            'student_id' => $this->session->userdata('student_id')
        ))->row();
...
$page_data['page_name']  = 'remita_tedc_fee_receipt';
    $page_data['page_title'] = 'FPN TEDC Fee Payment Receipt';
$this->load->view('backend/student_print',$page_data);
 }
...
$this->load->view('backend/student_print',$page_data);
	...
	function processRemitaAcceptanceInvoice($rrr){
...
$alreadyGenerated = $this->db->get_where('eduportal_remita_accp_temp_data', array("putme_id" => '1'))->row();
...
$pr= $studentDetails->programme;

		//define("MERCHANTID", $merchantId->value);

		define("MERCHANTID", 2266665151);

		//define("SERVICETYPEID", $serviceTypeId->value);
//define("APIKEY", $apiKey->value);

		define("APIKEY", 591873);
		if($pr==1)
		{
		define("SERVICETYPEID", 2223831078);
        $totalAmount =30000;
		}
		else{
	    define("SERVICETYPEID", 3429760973);
        $totalAmount =25000;
		}
...
define("PATH", 'http://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']));
		...
		$payerEmail = $studentDetails->email == "" ? 'info@ebsu.edu.ng' : $studentDetails->email;  

		$payerPhone = $studentDetails->phone; 

		$responseurl = "https://iportal.ebsu.edu.ng/portal/RemitaGatewayACP.php";
	
//header("Location: https://www.ebsu.edu.ng/portal/index.php?student/generate_acp_fee_invoice");
...
<?php
	...
	if($progtype == "DEGREE REGULAR"){

					$pType = "REGULAR";

				}
				if($progtype == "PART-TIME"){
				...
				$application_exist = $this->db->get_where('invoice_gen', array("portal_id" => $portalID,"application_type_id"=>'10',"session_id" => $session,"level" => $year))->row();
if($progtype == "PART-TIME"){
	$application_exist = $this->db->get_where('invoice_gen', array("portal_id" => $portalID,"application_type_id"=>'10',"session_id" => $session,"level" => $year,"paymentmode" => $paymentMode))->row();
}
if($application_exist)
{
	$invoice_data = array(
...
if($progtype == "PART-TIME"){
	define("SERVICETYPEID", 3429765633);
}
else{
	define("SERVICETYPEID", 3276893356);
}	//define("APIKEY", $apiKey->value);
...
$amount = $this->db->get_where('fedponek_fee_schedule', array("fee_type" =>2,"level"=> $year,"session" => $session,"dept_id" => $studentDetails->dept, "student_type_id" => $studentDetails->prog_type))->row();
    if($studentDetails->state=="Ebonyi")
    {
    $totalAmount= $amount-> indigene_amount;
      
    }
    else
    {
     $totalAmount= $amount-> nonindigene_amount;
      
    }
    if($studentDetails->dept_option=="4" || $studentDetails->dept_option=="9" ) 
	{
		 if($studentDetails->state=="Ebonyi")
      {
    $totalAmount= "84400";
     ...
     if($studentDetails->nationality=="Non-Nigerian")
    {
    $totalAmount= $amount->foreign_amount;
      
    }
	if($progtype == "PART-TIME"){
      if($paymentMode==1)
	  {
		  $totalAmount= 0.6 * $totalAmount;
		$_SESSION["totalAmount"]=$totalAmount;
	  }
	  if($paymentMode==2)
	  {
		  $totalAmount= 0.4 * $totalAmount;
		$_SESSION["totalAmount"]=$totalAmount;
	  }
	}
//echo $progtype.$paymentMode.SERVICETYPEID.$totalAmount.$studentDetails->prog_type.$studentDetails->dept;
//exit;
		$timesammp=DATE("dmyHis");		
		...
		$payerEmail = $studentDetails->email == "" ? 'student@ebsu.edu.ng' : $studentDetails->email;  
		...
		$itemid1="itemid1".$timesammp;

		$itemid2="344454".$itemtimestamp;

		$itemid3="86994".$itemtimestamp;
		
		$itemid4="38884".$itemtimestamp;
		
		

		$beneficiaryName="NEW SCHOOL FEE ACCOUNT EBONYI STATE UNIVERSITY";
		
		$beneficiaryName2="NEW SCHOOL FEE ACCOUNT EBONYI STATE UNIVERSITY";
		
	

		$beneficiaryAccount="4765382024";

		$beneficiaryAccount2="4765382024";
	...
	//	$beneficiaryAmount4 ="300";
		...
		$statusMsg = $response['status'];
		//print_r($jsonData);
//echo $statusMsg.$statuscode.$response;
//exit;
		if($statuscode=='025'){
...
if($paymentMode==1)
{
	$mode="FIRST INSTALLATION FEES";
}
if($paymentMode==2)
{
	$mode="SECOND INSTALLATION FEES";
}
if($paymentMode==3)
{
	$mode="FULL FEES";
...
$invoice_data = array(
			   ...
			   'email' => $email,
			   'dept_id' => $studentDetails->dept,
			   'fee_description'=>'EBSU SCHOOL FEE - '.$year.' '.$mode,
			   'level'=>$year,
			   'semester'=>$semester
...
$this->db->insert('invoice_gen',$invoice_data);	
  $_SESSION['invoice_no']=   $rrr;
		$page_data['page_name']  = 'invoice_page';
        $page_data['page_title'] = 'EBSU School Fee Payment Invoice';
	...
	public function remita_schfee_invoice()
{
	if ($this->session->userdata('student_login') != 1)
            redirect(base_url(), 'refresh');
$page_data['page_title'] = 'EBSU School Fee Invoice';
...
$this->load->view('backend/student_print',$page_data);
	...
	function remitaInvoice(){
        ...
        $page_data['page_title'] = get_phrase('EBSU Acceptance Fee Invoice');
...
$applicationcategory_code = $this->db->get_where('application_type', array("application_typeid" => $application_form_category))->row();
$lpad_invoice_id =$this->site_model->get_lpad_invoice_id(array('application_invoice_id'=>$application_invoice_id));
$invoice_code="FPN/".$applicationcategory_code->application_code."/17".$lpad_invoice_id;
...
public function etranzact_invoice()
{
	if ($this->session->userdata('student_login') != 1)
            redirect(base_url(), 'refresh');
$page_data['page_title'] = 'FPN HND Acceptance Fee Invoice';
...
$this->db->insert('invoice_gen',$invoice_data);	
  $_SESSION['invoice_no']=   $invoice_code;
		$page_data['page_name']  = 'invoice_page';
        $page_data['page_title'] = 'FPN HND School Fee Payment Invoice';
	...
	public function etranzact_schfee_invoice()
{
	if ($this->session->userdata('student_login') != 1)
            redirect(base_url(), 'refresh');
$page_data['page_title'] = 'FPN School Fee Invoice';
...
$page_data['page_name']  = 'remita_acp_receipt';
    $page_data['page_title'] = 'EBSU Acceptance Fee Payment Receipt';
$this->load->view('backend/student_print',$page_data);
...
$page_data['page_name']  = 'etransact_acp_receipt';
    $page_data['page_title'] = 'FPN HND Acceptance Fee Payment Receipt';
$this->load->view('backend/student_print',$page_data);
...
$page_data['page_name']  = 'remita_schfee_receipt';
    $page_data['page_title'] = 'EBSU School Fees Payment Receipt';
$this->load->view('backend/student_print',$page_data);
...
$page_data['page_name']  = 'remitaFailedPayment';
    $page_data['page_title'] = 'EBSU Payment Status';
$this->load->view('backend/student_print',$page_data);
...
$page_data['page_name']  = 'etransact_schfee_receipt';
    $page_data['page_title'] = 'FPN Fee Payment Receipt';
$this->load->view('backend/student_print',$page_data);
    ...
    function receiptprintout($param1 = ''){
        ...
        $page_data['page_title'] = get_phrase('EBSU School Fee Payment Receipt');
                    ...
                    if($this->etranzact->get_status() != "-1" && $this->etranzact->get_status() != '0' && $this->etranzact->get_confirm() != '0'){  //start etranzact check
                        ...
                        /*echo "am here";*/
                            ...
                            //var_dump($this->session->userdata('phone'));
                                ...
                                $this->ozioma->set_recipient($this->session->userdata('phone'));//separate numbers with commas aYEAR Include zip code in every number
   ...
   /******MANAGE OWN PROFILE AND CHANGE PASSWORD***/
    function manage_profile($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'update_profile_info') {
			...
			$oldRNo = $old0->reg_no;
			$inv['portal_id'] = $this->input->post('reg_no');
	    ...
	    $data['portal_id']   = $this->input->post('reg_no');
           ...
           // echo $this->input->post('nationality');
			...
			if($this->input->post('nationality')=='Nigerian')
			{
			...
			//EDU FEES PORTAL
           	$reg['regno']= $this->input->post('reg_no');
			$newRegNo = $this->input->post('reg_no'); 
	    	$this->db->where('student_id', $this->session->userdata('student_id'));
      ...
      if($imagesize > $max_file_size){
        $_SESSION['imgerror'] = "Passport image size is too large. Please upload an image less than 100kb";
...
$page_data['page_title'] = 'EBSU Students Information';
...
$page_data['page_title'] = 'FPN Students Information';
...
$this->load->view('backend/student_print',$page_data);
    ...
    function fetch_portal_info($payment_code){
		...
		//level
		if($student_id->payment_level == "YEAR I"){
			$fee_level_id = 1;
		}
		elseif($student_id->payment_level == "YEAR II"){
			$fee_level_id = 2;
		}
		elseif($student_id->payment_level == "HYEAR I"){
			$fee_level_id = 4;
		}
		else{
			$fee_level_id = 5;
...
$_SESSION["stuReg_id"]=$this->session->userdata('student_id');
   ...
   'programme_type_id'=> $this->input->post('programme_type_id')
	...
	{
	$_SESSION["error"]= "Sorry you have not made fee payment for this session !";
	$page_data['page_name']  = 'register_courses';
    $page_data['page_title'] = "Register Courses";
    $this->load->view('backend/index', $page_data);
