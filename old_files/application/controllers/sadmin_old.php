					...
					if($size<(2024*2024))
						{
							...
							if(move_uploaded_file($tmp, $path.$actual_image_name))
								...
								if($programme=="2")
								{
									$sql= "SELECT max(identification_no) as idm from eduportal_admission_list";
 	  $q =$this->db->query($sql);
       if($q->num_rows() > 0)
       {
 		...
 		foreach ($q->result() as $row)
          {
    $identification_no= $row->idm +1;
 //	$data[] = $row;
          }
 	  }

 	
		  $jambregno= "EBSU/WASP/".$identification_no;
		   $data["jambregno"] = $jambregno;


$this->db->where('id', $id);	  
$this->db->update('eduportal_admission_list_temp', $data);
	$jamb	  =$jambregno;
								}
...
<td><div align='center'><span class='style5'>DUPLICATE</span></div></td>
							...
							mysql_query("insert into `eduportal_admission_list` (application_no,surname,firstname,middlename,dept_id,school_id,dept_option_id,student_type,programme_type_id,session,admissionlist_batch_id,activated,user_id,adm_type,identification_no) values ('$jamb','$surname','$firstname','$middlename','$depts','$school','$deptsoptions','$programme','$programme_type_id','$sess','$batch','1','$sadmin_id','$ltype','$identification_no')") or die (mysql_error()."3");
...
echo		$_SESSION["error"];						
   ...
   $this->db->where('application_no', $id);
			...
			$this->db->where('application_no', $id);
            $this->db->update('ebsuedu1_admission.eduportal_admission_list', $data);
   ...
   $this->db->where('application_no', $id);
    $this->db->delete('eduportal_admission_list');
	   ...
	   $this->db->where('application_no', $id);
    $this->db->delete('ebsuedu1_admission.eduportal_admission_list');
