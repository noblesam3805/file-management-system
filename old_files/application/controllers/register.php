...
function processRemitaAcceptanceInvoice(){
		...
		if($programTpeID==4){
			...
			$aceptLimitCount->num_rows();
			 
			 if($aceptLimitCount==$limitValue || $aceptLimitCount>$limitValue ){
...
else
{
			...
			if($msg == 'Approved'){
               ...
               $student = $this->db->get_where('student', array("portal_id" => $stud->payerID))->row();
				...
				$data['channel'] = 'BRANCH';
				$data['amount'] = $response['amount'];
				$data['payer_name'] = $stud->payername;
				$data['payer_email'] =$stud->payeremail;

				$data['payer_phone'] = $stud->payerphone;
