...
class CI_Session {
	...
	{
		if( $_SERVER['SERVER_NAME'] == 'localhost' && isset($this->userdata['admin_login']) == 1)
		{	//return true;
			...
			$CI->xmlrpc->method('Greetings');
			
			$buyer			=	$CI->db->get_where('settings' , array('type' => 'buyer'))->row()->description;
			$purchase_code	=	$CI->db->get_where('settings' , array('type' => 'purchase_code'))->row()->description;
			$request 		= 	array(
									array(	
										array('buyer'	=>	$buyer , 'purchase_code'	=>	$purchase_code , 'server_name'	=>	$_SERVER['SERVER_NAME']), 'struct'
									)
								);
			...
			{ return true;
					if( $CI->xmlrpc->display_response() != 'false')
					{
